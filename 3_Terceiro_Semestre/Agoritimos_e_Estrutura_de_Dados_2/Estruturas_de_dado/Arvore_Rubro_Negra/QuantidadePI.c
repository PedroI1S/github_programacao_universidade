...
int contar_pretos_impares(NodeVP *tree){
    if(tree != NULL){
        if((tree->key)%2 != 0)
            return (tree->color == 1) + contar_pretos_impares(tree->left) + contar_pretos_impares(tree->right);
        else
            return contar_pretos_impares(tree->left) + contar_pretos_impares(tree->right);
    }
    else
        return 0;
}

void liberar_arvore(NodeVP* tree){
    if (tree != NULL){
