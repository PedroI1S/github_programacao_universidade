...
int paginas_cheias(NodeB *tree)
{
    while (!tree->eh_no_folha)
    {
        if (tree->nro_chaves == 5)
        {
            for (int i; i < tree->nro_chaves + 1; i++)
            {
                return 1 + paginas_cheias(tree->filhos[i]);
            }
        }
        else
        {
            for (int i; i < tree->nro_chaves + 1; i++)
            {
                return paginas_cheias(tree->filhos[i]);
            }
        }
    }
    return 0;
